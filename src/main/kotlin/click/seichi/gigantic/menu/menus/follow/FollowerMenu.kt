...
object FollowerMenu : BookMenu() {
    ...
    override fun setItem(inventory: Inventory, player: Player, page: Int): Inventory {
        ...
        (start until end)
                ...
                .toMap()
                .forEach { index, to ->
                    inventory.setItem(index, FollowSettingButtons.FOLLOWER_PLAYER(to).toShownItemStack(player))
                }
    ...
    override fun getButton(player: Player, page: Int, slot: Int): Button? {
        ...
        return when (slot) {
            ...
            else -> FollowSettingButtons.FOLLOWER_PLAYER(playerList.getOrNull(index) ?: return null)
        }
    }
