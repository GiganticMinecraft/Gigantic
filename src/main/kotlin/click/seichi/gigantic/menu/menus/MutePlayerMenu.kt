 ...
 * @author tar0ss
 */
object MutePlayerMenu : BookMenu() {
    ...
    override fun setItem(inventory: Inventory, player: Player, page: Int): Inventory {
        ...
        (start until end)
                ...
                .toMap()
                .forEach { index, to ->
                    inventory.setItem(index, FollowSettingMenuButtons.MUTE_ONLINE_PLAYER(to).findItemStack(player))
                }
    ...
    override fun getTitle(player: Player, page: Int): String {
        return "${FollowSettingMenuMessages.MUTE_ONLINE.asSafety(player.wrappedLocale)} $page/${getMaxPage(player)}"
    }
