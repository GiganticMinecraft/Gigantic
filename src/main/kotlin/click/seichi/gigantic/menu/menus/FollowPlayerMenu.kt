...
object FollowPlayerMenu : BookMenu() {
    ...
    override fun setItem(inventory: Inventory, player: Player, page: Int): Inventory {
        ...
        (start until end)
                ...
                .toMap()
                .forEach { index, to ->
                    inventory.setItem(index, FollowSettingMenuButtons.FOLLOW_ONLINE_PLAYER(to).toShownItemStack(player))
                }
    ...
    override fun getButton(player: Player, page: Int, slot: Int): Button? {
        ...
        return when (slot) {
            ...
            else -> FollowSettingMenuButtons.FOLLOW_ONLINE_PLAYER(playerList.getOrNull(index) ?: return null)
        }
    }
